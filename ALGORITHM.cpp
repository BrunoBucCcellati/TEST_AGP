...
double Shag(double _m, double x1, double x2, double y1, double y2, unsigned short _N)
{
    if (_N == 1)
    {
        return 0.5 * (x1 + x2) - Sign(y2 - y1) * 0.5 * abs(y2 - y1) / _m;
    }
    return 0.5 * (x1 + x2) - Sign(y2 - y1) * 0.25 * (y2 - y1) * (y2 - y1) / (_m * _m);
}
class Interval
{
private:
...
typedef std::priority_queue<Interval, std::deque<Interval>, Compare> Mypriority_queue;
