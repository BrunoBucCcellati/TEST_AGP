...
namespace TESTAGP {
	...
	{
	public:
		cliext::deque<double> Base_LNA_1_2_Mer_AGP(time_t now, bool mode, unsigned short N, double b, PeanoCurve_2D^ Curve, PeanoCurve_2D^ Curve_Minus_PI_Na_Dva, double r, double epsilon, unsigned short global_local_iterations, double a, double c, double d)
		{
			...
			if (N == 1)
			{
				...
				{
					pred_i_sled_shag.first = pred_i_sled_shag.second, promejutochnaya_tochka.first = pred_i_sled_shag.second = Shag(m, x_Rmax.first, x_Rmax.second, y_Rmax.first, y_Rmax.second, N), promejutochnaya_tochka.second = ShekelFunc(pred_i_sled_shag.second, now), Extr.push_back(promejutochnaya_tochka.second);
					if (schetchick == 1000)
					{
						return Extr;
					}
			...
			{
				HINSTANCE load_function = LoadLibrary(L"TEST_FUNC.dll"); typedef double (*grsh) (double, double, time_t); grsh GrishaginFunc = (grsh)GetProcAddress(load_function, "GrishaginFunc"); start = std::pair<double, double>(a, GrishaginFunc(a, c, now)), end = std::pair<double, double>(b, GrishaginFunc(b, c, now)), start_Minus_PI_Na_Dva = std::pair<double, double>(a, GrishaginFunc(a, c, now)), end_Minus_PI_Na_Dva = std::pair<double, double>(b, GrishaginFunc(a, d, now));
				...
				while (abs(pred_i_sled_shag.second - pred_i_sled_shag.first) > epsilon)
				{
					pred_i_sled_shag.first = pred_i_sled_shag.second, pred_i_sled_shag.second = Shag(m, x_Rmax.first, x_Rmax.second, y_Rmax.first, y_Rmax.second, N), pred_i_sled_shag_Minus_PI_Na_Dva.first = pred_i_sled_shag_Minus_PI_Na_Dva.second, pred_i_sled_shag_Minus_PI_Na_Dva.second = Shag(m_Minus_PI_Na_Dva, x_Rmax_Minus_PI_Na_Dva.first, x_Rmax_Minus_PI_Na_Dva.second, y_Rmax_Minus_PI_Na_Dva.first, y_Rmax_Minus_PI_Na_Dva.second, N),  promejutochnaya_tochka.second = GrishaginFunc(Curve->HitTest_2D(pred_i_sled_shag.second).first, Curve->HitTest_2D(pred_i_sled_shag.second).second, now); promejutochnaya_tochka_Minus_PI_Na_Dva.second = GrishaginFunc(Curve_Minus_PI_Na_Dva->HitTest_2D(pred_i_sled_shag_Minus_PI_Na_Dva.second).first, Curve_Minus_PI_Na_Dva->HitTest_2D(pred_i_sled_shag_Minus_PI_Na_Dva.second).second, now), promejutochny_otrezok = R.top(), promejutochny_otrezok_Minus_PI_Na_Dva = R_Minus_PI_Na_Dva.top();
					Extr.push_back((std::min)(promejutochnaya_tochka.second, promejutochnaya_tochka_Minus_PI_Na_Dva.second));
					if (schetchick == 1000)
					{
						return Extr;
					}
...
#pragma region Windows Form Designer generated code
		   void InitializeComponent(void)
		   {
			   ...
			   chartArea1->AxisX->LabelStyle->Font = (gcnew System::Drawing::Font(L"MS Gothic", 12, System::Drawing::FontStyle::Bold, System::Drawing::GraphicsUnit::Point,
			   ...
			   chartArea1->AxisY->LabelAutoFitMinFontSize = 18;
			   ...
			   this->chart1->Size = System::Drawing::Size(2206, 796);
			   ...
			   this->ClientSize = System::Drawing::Size(2230, 820);
...
#pragma endregion
	private:
		...
		cli::array<unsigned short>^ procent_correct_2D = gcnew cli::array<unsigned short>(1001);
		cli::array<unsigned short>^ procent_correct_LNA_2D = gcnew cli::array<unsigned short>(1001);
		...
		{
			unsigned short num = 0, MIN_LNA_SIZE = 500, MIN_LNA_SIZE_2D = 500;
			while (num < 1000)
			{
				...
				FreeLibrary(load_function);
				Extr_2D[num] = gcnew cliext::deque<double>(Base_LNA_1_2_Mer_AGP(now, false, 2, 1, Curve, Curve_Minus_PI_Na_Dva, 2, 5 * pow(10, -15), 38, 0, 0, 1));
				Extr_LNA_2D[num] = gcnew cliext::deque<double>(Base_LNA_1_2_Mer_AGP(now, true, 2, 1, Curve, Curve_Minus_PI_Na_Dva, 2.8, 5 * pow(10, -15), 198, 0, 0, 1));
				Extr[num] = gcnew cliext::deque<double>(Base_LNA_1_2_Mer_AGP(now, false, 1, 10, nullptr, nullptr, 3, 5 * pow(10, -15), 38, 0, 0, 1));
				...
				while (Extr[num]->empty() == false)
				{
					if (Extr_2D[num]->front() < MIN_GRSH || Extr_2D[num]->front() - MIN_GRSH < 0.1)
					{
					...
					{
						if (Extr_LNA_2D[num]->front() < MIN_GRSH || Extr_LNA_2D[num]->front() - MIN_GRSH < 0.1)
						{
